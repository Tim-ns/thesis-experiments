...
from datasets import load_dataset, GeneratorBasedBuilder
...
class LMDataModule:
    ...
    def prepare_data(self):
        if self.cache_dir is None:
            ...
            load_dataset(self.dataset_name, self.dataset_config_name, trust_remote_code=True)
        else:
    ...
    def setup(self, stage=None):
        ...
        self.dataset_train, self.dataset_val, self.dataset_test = [
            LMDataset(concat_ids[split], seq_len=self.max_length, bos_token_id=getattr(self.tokenizer, "bos_token_id", None))
    ...
    def process_dataset(self):
        ...
        if self.raw_json_path is not None:
        ...
        elif self.dataset_name.endswith(".py") and osp.isfile(self.dataset_name):
            # Manually load the dataset builder from the script because load_dataset disables scripts
            module_name = "dataset_script"
            spec = importlib.util.spec_from_file_location(module_name, self.dataset_name)
            module = importlib.util.module_from_spec(spec)
            sys.modules[module_name] = module
            spec.loader.exec_module(module)

            # Find the builder class in the module
            builder_class = None
            ...
            if builder_class is None:
                raise RuntimeError(f"Could not find a GeneratorBasedBuilder in {self.dataset_name}")

            builder = builder_class(config_name=self.dataset_config_name)
            builder.download_and_prepare()
            raw_datasets = builder.as_dataset()
        else:
            raw_datasets = load_dataset(self.dataset_name, self.dataset_config_name, trust_remote_code=True)
