...
# Experiment details
EXP_NAME="bookcorpus_${MODEL_SIZE}_transformer_${TOTAL_STEPS}_steps"
EXP_DIR="${EXP_ROOT}/ttt-experiments"

mkdir -p "${EXP_DIR}/${EXP_NAME}"

mkdir -p ttt-lm/logs/training/transformer
LOG_FILE="ttt-lm/logs/training/transformer/train_$(date +%Y%m%d_%H%M%S).log"

echo "Starting Transformer training with:"
...
python3 -m ${DEPS_DIR}.ttt-lm-jax.ttt.train \
        --mesh_dim='1,1,-1' \
        --dtype='bf16' \
        --update_model_config="dict(remat_block='nothing_saveable', scan_attention=False, scan_mlp=False)" \
        --total_steps=${TOTAL_STEPS} \
        --save_checkpoint_freq=2500 \
        --save_milestone_freq=5000 \
        --load_model_config=${MODEL_SIZE} \
